...
from app.integrations.postgres.dtos.user_dto import UserDetailDTO
...
async def test_target_user_with_rank(
    ...
    assert await user_repository.get_one_with_rank(user_id=user.id) == UserDetailDTO.model_validate(user)
...
async def test_nonexistent_user(
    ...
    with pytest.raises(UserNotFoundPostgres):
        await user_repository.get_one_with_rank(user_id=1)
