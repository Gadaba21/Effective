...
from app.integrations.postgres.dtos.user_dto import UserLoginResponseDTO
...
async def test_target_user(
    ...
    assert await user_repository.get_one_by_username(username=user.username) == UserLoginResponseDTO.model_validate(
        user
    )
...
async def test_nonexistent_user(
    ...
    assert await user_repository.get_one_by_username(username='test') is None
