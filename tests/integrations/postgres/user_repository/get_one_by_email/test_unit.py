...
async def test_target_user(
    ...
    assert await user_repository.get_one_by_email(email=user.email) == UserByEmailDTO.model_validate(user)
...
async def test_nonexistent_user(
    ...
    with pytest.raises(UserNotFoundPostgres):
        await user_repository.get_one_by_email(email='test@example.com')
