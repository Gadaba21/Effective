...
async def test_change_max_players(
    ...
    assert room.max_players == room_orm.max_players

    await lobby_repository.change_max_players(room_id=room.id, max_players=4)
    room_orm = await fake_session.scalar(select(RoomORM).where(RoomORM.id == room.id))
    assert room_orm.max_players == 4
