...
async def test_game_started(
    ...
    assert room.started == room_orm.started

    await lobby_repository.game_started(room_id=room.id, started=True)
    room_orm = await fake_session.scalar(select(RoomORM).where(RoomORM.id == room.id))
    assert room_orm.started == True

    await lobby_repository.game_started(room_id=room.id, started=False)
    room_orm = await fake_session.scalar(select(RoomORM).where(RoomORM.id == room.id))
    assert room_orm.started == False
