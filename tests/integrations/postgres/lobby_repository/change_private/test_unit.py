...
async def test_change_private(
    ...
    await lobby_repository.change_private(room_id=room.id, is_private=False)
    room_orm = await fake_session.scalar(select(RoomORM).where(RoomORM.id == room.id))
    assert room_orm.is_private is False

    await lobby_repository.change_private(room_id=room.id, is_private=True)
    room_orm = await fake_session.scalar(select(RoomORM).where(RoomORM.id == room.id))
    assert room_orm.is_private is True
